...
public class Pageable<T> implements IPageable<T> {
    ...
    public Pageable(long current, long size, boolean isSearchCount) {
        this(current, size, 0, isSearchCount);
    }

    public Pageable(long current, long size, long total, boolean isSearchCount) {
        if (current > 1) {
     ...
     * @return 返回分页参数本身
     */
    public Pageable<T> addOrder(OrderItem... items) {
     ...
     * @return 返回分页参数本身
     */
    public Pageable<T> addOrder(List<OrderItem> items) {
        orders.addAll(items);
        return this;
    }
    ...
    public Pageable<T> setOptimizeCountSql(boolean optimizeCountSql) {
